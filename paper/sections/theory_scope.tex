...
\paragraph{Proposition 1 (set geometry and exact coverage event).}
...
\paragraph{Proposition 2 (deterministic coverage-deviation bound).}
...
finite-sample-corrected order statistic while the MVP weighted rule uses the
...
\paragraph{Corollary (ranking invariance).}
...
implemented empirical quantile and makes clear which terms the route-A
